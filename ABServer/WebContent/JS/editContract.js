...
contractProxy = new Ext.data.HttpProxy({
 ...
 contractDS.on('load', function(){
		...
		createProposalForm.setTitle('Edit Contract - '+contractRec.get('clientName'));
...
var coordinatorsteamsDS = new Ext.data.Store({
  ...
  var rateTypeData = [
	...
	var contractRateCombo = new Ext.form.ComboBox({            	
		    store: rateTypeDS,
		    fieldLabel: 'Contract Rate Type <html><font color=red> *</font></html>',
		    displayField:'name',
		    width: 200,
		    valueField: 'id',
		    selectOnFocus: true,
...
/////////////////////////////////////assign coordinator/////////////////////////////////////////////////////////////////
  ...
  var coordinator_row_index = 0;
  
  
  
  var assignCoordinatorsDS = new Ext.data.Store({
       // load using HTTP
      proxy: retreivingDataProxy,
     // baseParams:{task: "coordinators"},
      // the return will be XML, so lets set up a reader
	  ...
	  var coordinatorsCombo = new Ext.form.ComboBox({
	     ...
	     store: assignCoordinatorsDS,
		 fieldLabel: 'Coordinator Name',
	//	 valueField: 'idTrainingCoordinators',
	     displayField:'coordinatorName',
		 selectOnFocus: true,
		 typeAhead: true,
	     editable: true,
	    ...
	    coordinatorGrid.getStore().getAt(coordinator_row_index).set('coordinatorId',assignCoordinatorsDS.getAt(index).get('coordinatorId'));	
	     }}
	});

 var coordinatorcm = new Ext.grid.ColumnModel([{
           id:'courseId',
           header: "Course Name",
           dataIndex: 'courseName',
           width: 220
           ...
           width: 230,
           editor: coordinatorsCombo
        }
    ]);



 var coordinatorGrid = new Ext.grid.EditorGridPanel({
        store: ds,
        cm: coordinatorcm,
    ...
    coordinatorGrid.on('cellclick',function(grid, rowIndex, columnIndex, e) {
//        var record = grid.getStore().getAt(rowIndex);  // Get the Record
		coordinator_row_index = rowIndex;
//		  //console.log('name:   '+record.get('coordinatorName'));
		
		  if(getcoordinatorSelectionValue() == 2)
		  assignCoordinatorsDS.load({params:{task:'coordinatorsByTeamId',team_id:coordinatorTeamId}});
		  
		  else
		    assignCoordinatorsDS.load({params:{task:'coordinators'}});
				  
	
});


var saveAssignCoordinator = function(){
...
//	ds.commitChanges();
	...
	for(var index=0; index<coordinatorGrid.getStore().getCount(); index++) {
		
		coordinatorsIds[index] = coordinatorGrid.getStore().getAt(index).get('coordinatorId');
		courseIds[index] = coordinatorGrid.getStore().getAt(index).get('courseId');
	...
	Ext.Ajax.request({   
        ...
        url: '../ContractsAction.do',
        params: {
          task: "assignCoordinatorToConractCourse",
          coordinators_ids:  coordinatorsIds,
          course_ids:courseIds
        },
        method:'POST', 
        success: function(response){
   ...
   //   layout: 'fit',
      items: [venueDatashowForm],
         buttons:[{ 
            	...
            	handler:function(){//venue_datashow_Grid.getStore().load();
            	assignVenueDatashowWindow.hide();}
            	}
           ] 
    });
...
/////////////////////////////////////LOC Tab/////////////////////////////////////////////////////////////////
 ...
 // custom column plugin example
	var locReceivedCheckColumn = new Ext.grid.CheckColumn({
	   header: "LOC Received",
	   dataIndex: 'locReceived',
	   width: 155
	});

var CPRReceivedCheckColumn = new Ext.grid.CheckColumn({
	   header: "CPR Received",
	   dataIndex: 'CPRReceived',
	   width: 155
	});
 ...
 var loccm = new Ext.grid.ColumnModel([{
           ...
           dataIndex: 'status',
           width: 100
        },locReceivedCheckColumn,{
           header: "LOC Receiving Date",
           dataIndex: 'locReceivedDate',
           width: 195,
           renderer: formatDate,
        ...
        },CPRReceivedCheckColumn,{
           header: "CPR Receiving Date",
           dataIndex: 'CPRReceivedDate',
           width: 195,
           renderer: formatDate,
 ...
 var locGrid = new Ext.grid.EditorGridPanel({
        store: ds,
        cm: loccm,
        ...
        title:'Edit LOC',
        frame:true,
        plugins:[locReceivedCheckColumn,CPRReceivedCheckColumn],
        
        clicksToEdit:1
    });

var saveLOC = function(){
	
	var loc_received = []; 
	var courseIds = [];
	var loc_received_dates = [];
	var loc_numbers = [];
	
	var cpr_received = [];
	var cpr_received_dates = [];
	
	var error = false;
	
	for(var index=0; index<locGrid.getStore().getCount(); index++) {
		
		
		if(locGrid.getStore().getAt(index).get('locReceived') == true && locGrid.getStore().getAt(index).get('locReceivedDate') == ''){
			
			Ext.Msg.show({
	         title: 'Error', 
	        msg: 'You Must Enter The Date of LOC Receiving of '+locGrid.getStore().getAt(index).get('courseName')+' Course with ('+locGrid.getStore().getAt(index).get('courseRuns')+') Run Number',
	        minWidth: 200,
	        modal: true,
	        icon: Ext.Msg.ERROR,
	        buttons: Ext.Msg.OK
		...
		if(locGrid.getStore().getAt(index).get('locReceived') == true)
		loc_received[index] = 1;
		else
		loc_received[index] = 0;
		
		courseIds[index] = locGrid.getStore().getAt(index).get('courseId');
			
		if(locGrid.getStore().getAt(index).get('locReceivedDate') != null && locGrid.getStore().getAt(index).get('locReceivedDate') != ''){
		loc_received_dates[index] = returnFormatDate(locGrid.getStore().getAt(index).get('locReceivedDate'));
		loc_received[index] = 1;
		}
		else
		loc_received_dates[index] = locGrid.getStore().getAt(index).get('locReceivedDate');
		
		loc_numbers[index] = locGrid.getStore().getAt(index).get('locNumber');
		
		
		if(locGrid.getStore().getAt(index).get('CPRReceived') == true && locGrid.getStore().getAt(index).get('CPRReceivedDate') == ''){
			
			Ext.Msg.show({
	         title: 'Error', 
	        msg: 'You Must Enter The Date of CPR Receiving of '+locGrid.getStore().getAt(index).get('courseName')+' Course with ('+locGrid.getStore().getAt(index).get('courseRuns')+') Run Number',
	        minWidth: 200,
	        modal: true,
	        icon: Ext.Msg.ERROR,
	        buttons: Ext.Msg.OK
		...
		if(locGrid.getStore().getAt(index).get('CPRReceived') == true)
		cpr_received[index] = 1;
		else
		cpr_received[index] = 0;
		
		if(locGrid.getStore().getAt(index).get('CPRReceivedDate') != null && locGrid.getStore().getAt(index).get('CPRReceivedDate') != ''){
		cpr_received_dates[index] = returnFormatDate(locGrid.getStore().getAt(index).get('CPRReceivedDate'));
		cpr_received[index] = 1;
		}
		else
		cpr_received_dates[index] = locGrid.getStore().getAt(index).get('CPRReceivedDate');
	}
	...
	Ext.Ajax.request({   
        ...
        url: '../ContractsAction.do',
        params: {
          task: "saveLOCToContractCourse",
          locReceived:       loc_received,
          course_ids:        courseIds,
          locReceivedDates:  loc_received_dates,
          locNumbers:        loc_numbers,
          cprReceived:       cpr_received,
          cprReceivedDates:  cpr_received_dates
        },
        method:'POST', 
        success: function(response){
        	ds.commitChanges();
...
/////////////////////////////////////confirmations tab/////////////////////////////////////////////////////////////////
 ...
 // custom column plugin example
	var clientConfirmCheckColumn = new Ext.grid.CheckColumn({
	   header: "Client Confirm",
	   dataIndex: 'clientConfirm',
	   width: 155
	});

var CancelCourseCheckColumn = new Ext.grid.CheckColumn({
	   header: "Cancel Course",
	   dataIndex: 'courseCancel',
	   width: 155
	});

 var confirmations_cm = new Ext.grid.ColumnModel([{
           id:'courseId',
           header: "Course Name",
           dataIndex: 'courseName',
           width: 220
           ...
           dataIndex: 'status',
           width: 100
        },clientConfirmCheckColumn,{
           header: "Client Confirm Date",
           dataIndex: 'clientConfirmDate',
           width: 195,
           renderer: formatDate,
        ...
        },CancelCourseCheckColumn,{
           header: "Course Cancelling Date",
 ...
 var confirmations_Grid = new Ext.grid.EditorGridPanel({
        store: ds,
        cm: confirmations_cm,
        ...
        title:'Confirmations & Course Cancelling',
        frame:true,
        plugins:[clientConfirmCheckColumn,CancelCourseCheckColumn],
        
        clicksToEdit:1
    });

var save_confirmations = function(){
...
//	ds.commitChanges();
	...
	var client_confirm = []; 
	var courseIds = [];
	var client_confirm_dates = [];
	
	var course_cancelled = [];
	var course_cancelled_dates = [];
	
	var error = false;
	
	for(var index=0; index<confirmations_Grid.getStore().getCount(); index++) {
		
		
		if(confirmations_Grid.getStore().getAt(index).get('clientConfirm') == true && confirmations_Grid.getStore().getAt(index).get('clientConfirmDate') == ''){
			
			Ext.Msg.show({
	         title: 'Error', 
	        msg: 'You Must Enter The Date of Client Confirmation of '+confirmations_Grid.getStore().getAt(index).get('courseName')+' Course with ('+confirmations_Grid.getStore().getAt(index).get('courseRuns')+') Run Number',
	        minWidth: 200,
	        modal: true,
	        icon: Ext.Msg.ERROR,
	        buttons: Ext.Msg.OK
		...
		if(confirmations_Grid.getStore().getAt(index).get('clientConfirm') == true)
		client_confirm[index] = 1;
		else
		client_confirm[index] = 0;
		
		courseIds[index] = confirmations_Grid.getStore().getAt(index).get('courseId');
		
		if(confirmations_Grid.getStore().getAt(index).get('clientConfirmDate') != null && confirmations_Grid.getStore().getAt(index).get('clientConfirmDate') != ''){
		client_confirm_dates[index] = returnFormatDate(confirmations_Grid.getStore().getAt(index).get('clientConfirmDate'));
		client_confirm[index] = 1;
		}
		else
		client_confirm_dates[index] = confirmations_Grid.getStore().getAt(index).get('clientConfirmDate');
		
		if(confirmations_Grid.getStore().getAt(index).get('courseCancel') == true && confirmations_Grid.getStore().getAt(index).get('cancelDate') == ''){
			
			Ext.Msg.show({
	         title: 'Error', 
	        msg: 'You Must Enter The Date of Client Cancelling of '+confirmations_Grid.getStore().getAt(index).get('courseName')+' Course with ('+confirmations_Grid.getStore().getAt(index).get('courseRuns')+') Run Number',
	        minWidth: 200,
	        modal: true,
	        icon: Ext.Msg.ERROR,
	        buttons: Ext.Msg.OK
		...
		if(confirmations_Grid.getStore().getAt(index).get('courseCancel') == true)
		course_cancelled[index] = 1;
		else
		course_cancelled[index] = 0;
		
		if(confirmations_Grid.getStore().getAt(index).get('cancelDate') != null && confirmations_Grid.getStore().getAt(index).get('cancelDate') != ''){
		course_cancelled_dates[index] = returnFormatDate(confirmations_Grid.getStore().getAt(index).get('cancelDate'));
		course_cancelled[index] = 1;
		}
		else
		course_cancelled_dates[index] = confirmations_Grid.getStore().getAt(index).get('cancelDate');
	}
	...
	Ext.Ajax.request({   
        ...
        url: '../ContractsAction.do',
        params: {
          task: "assignClientCofirmationToContractCourse",
          clientConfirm:        client_confirm,
          course_ids:           courseIds,
          contractId:contractId,
          clientConfirmDates:   client_confirm_dates,
          courseCancel:         course_cancelled,
          courseCancelDates:    course_cancelled_dates
        },
        method:'POST', 
        success: function(response){
        	ds.commitChanges();
...
var pan= new Ext.TabPanel({
                    ...
                    items:[createProposalForm,contractCoursesGrid,resourceGrid,coordinatorGrid,venue_datashow_Grid,locGrid,confirmations_Grid],
     		buttons:[{ 
                text:'Save Changes',
                iconCls:'save',
                formBind: true,  
                ...
                handler:function(){ 
                 if(pan.activeTab == createProposalForm){
                 	...
                 	if(clientApproveDateField.getValue() != '')
      				...
      				else if(pan.activeTab == coordinatorGrid){
      					saveAssignCoordinator();
      					//contractDS.load();
      				}
      				...
      				else if(pan.activeTab == locGrid){
      					saveLOC();
      					//contractDS.load();
      				}
      				else if(pan.activeTab == confirmations_Grid){
      					save_confirmations();
      					//contractDS.load();
      				}
