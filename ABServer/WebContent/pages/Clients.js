...
Ext.onReady(function() {
    ...
    var ds = new Ext.data.Store({
      ...
      reader: new Ext.data.XmlReader({
        ...
        sortInfo:{field: "clientName", direction: "ASC"}
      });
    ...
    // custom or reusable ColumnModels
      var colModel = new Ext.grid.ColumnModel([
      	{header: "Client Company Name", width: 130, sortable: true, locked:false, dataIndex: 'clientName'}, 
        {header: "Client Abbreviation", width: 130, sortable: true, locked:false, dataIndex: 'clientApp'}, 
        {header: "Main Contact First Name", width: 150, sortable: true, dataIndex: 'personFName'},
        {header: "Main Contact Last Name", width: 150, sortable: true, dataIndex: 'personLName'},
        {header: "Main Contact  Title", width: 130, sortable: true, locked:false, dataIndex: 'personTitle'},
        {header: "Main Contact  Departnment", width: 150, sortable: true, locked:false, dataIndex: 'personDep'},
        {header: "Main Contact Telephone ", width: 130, sortable: true, locked:false, dataIndex: 'personTelePhone'}, 
        {header: "Main Contact Mobile ", width: 130, sortable: true, dataIndex: 'personMobile'},
        {header: "Main Contact E-Mail ", width: 130, sortable: true, dataIndex: 'personEmail'}
 ...
 var filters = new Ext.grid.GridFilters({
	  filters:[
	    ...
	    {type: 'string',  dataIndex: 'clientName'}
...
//myGrid.getView().refresh();
     ...
     //   title:'Add Coordinator History',
        ...
        items: [{
         ...
         buttons:[{ 
            	...
            	handler:function(){AddClientWindow.hide();resetCourseForm();}
            	}
           ] 
 ...
 function resetCourseForm(){
  ...
  return(ClientNameField.isValid() && ClientAddField.isValid() && ClientAppField.isValid() && ClientAppDateField.isValid() && WorkDateField.isValid() && AppPersonField.isValid() && CFNameField.isValid() && CLNameField.isValid() && CAddField.isValid() && CMailField.isValid()&& CTelephoneField.isValid() && CMobileField.isValid()&& CTitleField.isValid());
  }
  ...
  if(ClientAppDateField.getValue()!='')
  {
   ...
   if(ClientNameField.isValid() && ClientAddField.isValid() && ClientAppField.isValid() && ClientAppDateField.isValid() && WorkDateField.isValid() && AppPersonField.isValid() && CFNameField.isValid() && CLNameField.isValid() && CAddField.isValid() && CMailField.isValid()&& CTelephoneField.isValid() && CMobileField.isValid()&& CTitleField.isValid()){
      Ext.Ajax.request({   
        waitMsg: 'Please wait...',
        url: '../listClients.do',
        params: {
 ...
 /********************************************************************/
  ...
  function confirmDeleteCourses(){
    ...
    {
      Ext.MessageBox.confirm('Confirmation','Are you sure you want to delete this Client?', deleteCourses);
    } else if(myGrid.selModel.getCount() > 1){
      Ext.MessageBox.confirm('Confirmation','Are you sure you want to delete those Clients?', deleteCourses);
    } else {
