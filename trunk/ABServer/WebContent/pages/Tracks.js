...
Ext.onReady(function() {
    ...
    var ds = new Ext.data.Store({
      ...
      reader: new Ext.data.XmlReader({
        ...
        sortInfo:{field: "trackName", direction: "ASC"}
      });
    ...
    // custom or reusable ColumnModels
      var colModel = new Ext.grid.ColumnModel([
      	{header: "Track Name", width: 150, sortable: true, dataIndex: 'trackName'},
		{header: "Track Code", width: 200, sortable: true, dataIndex: 'trackCode'},
		{header: "Track Duration", width: 200, sortable: true, dataIndex: 'trackDuration'}
    ]);
...
window.location = 'editTrack.jsp?t='+selections[0].id;
});
...
//    maxLength: 100,
  ...
  function addTA(){
   ...
   if(AreaNameField.isValid() && AreaCodeField.isValid()){
      Ext.Ajax.request({   
        ...
        params: {
          task: "AddTrack",
          trackName:      AreaNameField.getValue(),
          trackCode:      AreaCodeField.getValue()
        },
        method:'POST', 
 ...
 /********************************************************************/
  ...
  var selectedCourse = [];
  for(i = 0; i< myGrid.selModel.getCount(); i++){
    selectedCourse.push(selections[i].xml.idTracks);
  }
  ...
  function confirmDeleteCourses(){
    ...
    {
      Ext.MessageBox.confirm('Confirmation','Do you not like that Track at all?', deleteCourses);
    } else if(myGrid.selModel.getCount() > 1){
      Ext.MessageBox.confirm('Confirmation','Are you sure you want to delete those Tracks?', deleteCourses);
    } else {
